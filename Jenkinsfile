        ...
        stage('Deploye Code') {
            if (isUnix()) {
                rc = sh returnStatus: true, script : "${toolbelt} forceauthjwtgrant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${SFDC_HOST}"
                 ...
                 rc = bat returnStatus : true, script :"sfdx forceauthjwtgrant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${SFDC_HOST}"
            }
